...
if (isset($_SESSION['session']) && $_SESSION['session'] -> sessionIsValid()) {
	...
	$applications_db =  new ApplicationsDatabase();
	$applications = $applications_db -> getApplications();
	...
	if (isset($_POST['deleted_files']) && is_array($_POST['deleted_files'])) {
		$applications_db -> deleteApplications($_POST['deleted_files']);
		header('Location: ' . $_SERVER['REQUEST_URI']);
	}
	...
	function render_tbody_from_files() {
		...
		foreach ($applications as $application) {
			$value .= '<tr>';
			$value .= '<td><input type="checkbox" name="deleted_files[]" value="' . $application['id'] . '"></td>';
			...
			$value .= '<td>' . $application['payment_method'] . '</td>';
